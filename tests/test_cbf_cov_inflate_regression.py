...
precision" on this same numpy/scipy combination). This test did not exist
before; it exists so an upcoming Clarabel solver swap (see PROGRESS.md/
CLAUDE.md) has something concrete to fail if it silently changes behavior.
...
Slow: runs several thousand SLSQP-backed rollout steps (the k_alpha_base=3.0
...
def _run_cov_inflate(splats, geom_filt, k_alpha_base, cov_gamma):
    cfg_dict = {**BASE_CFG_DICT, "k_alpha_base": k_alpha_base}
...
def test_step0_narrow_corridor_gamma1_pathological_slowdown(narrow_splats, narrow_geom_filt, oracle_time):
    """PROGRESS.md 'Step 0' / reproduced 'Step 0/1' table, narrow corridor,
    COV_INFLATE gamma=1.0, k_alpha_base=1.0 (config default):
    severity=+0.165, time_ratio=11.91, infeasible_count=0."""
    result, severity, actual_time = _run_cov_inflate(narrow_splats, narrow_geom_filt, k_alpha_base=1.0, cov_gamma=1.0)
    time_ratio = actual_time / oracle_time

    assert result.reached_goal
    assert result.infeasible_count == 0
    assert severity == pytest.approx(0.165, abs=5e-4)
    assert time_ratio == pytest.approx(11.91, abs=5e-3)


def test_step2_wide_corridor_gamma1_time_ratio_collapses(wide_splats, wide_geom_filt, oracle_time):
    """PROGRESS.md 'Step 2': identical gamma/k_alpha_base, wider corridor --
    the 11.91x slowdown collapses to ~1.09x."""
    result, _, actual_time = _run_cov_inflate(wide_splats, wide_geom_filt, k_alpha_base=1.0, cov_gamma=1.0)
    time_ratio = actual_time / oracle_time
...
def test_step3_narrow_corridor_k_alpha_sweep_infeasible_count(
    narrow_splats, narrow_geom_filt, k_alpha_base, expected_infeasible_count
):
    ...
    result, _, _ = _run_cov_inflate(narrow_splats, narrow_geom_filt, k_alpha_base=k_alpha_base, cov_gamma=1.0)
